...
int main(int argc, char* argv[]) {
    try {
        ...
        while (std::getline(fin, line)) {
            ...
            // --- Filtering (adaptive cutoff using spectrum-energy knee/elbow) ---
            double fs_rate = 120.0; // Hz

            // estimate adaptive cutoff (clamped internally to sensible bounds)
            double cutoff = AutoCutoff::estimateCutoffKneePoint(acc_y, fs_rate, 3.0, 10.0, 5.0);
            std::cout << "Adaptive cutoff (Hz): " << cutoff << "\n";
