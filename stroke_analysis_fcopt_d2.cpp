...
int main(int argc, char* argv[]) {
    try {
        ...
        options.add_options()
            ...
            ("f,folder", "Input folder containing CSV files", cxxopts::value<std::string>())
   ...
   (!result.count("input") && !result.count("folder"))) {
...
if (result.count("input")) {
    input_files.push_back(result["input"].as<std::string>());
}
...
for (const auto& csv_path : input_files) {

    std::cout << "Processing: " << csv_path << std::endl;

    std::ifstream fin(csv_path);   // <<-- keep only this declaration
    if (!fin.is_open()) {
    ...
    int idx_AccX = find_idx("linear_acceleration_x");
    int idx_AccY = find_idx("linear_acceleration_y");
    int idx_GyrX = find_idx("angular_velocity_x");
    int idx_GyrY = find_idx("angular_velocity_y");
    int idx_GyrZ = find_idx("angular_velocity_z");
    ...
    while (std::getline(fin, line)) {
        ...
        std::string cell;
        std::vector<std::string> row_cells;  // removed unused col_index
